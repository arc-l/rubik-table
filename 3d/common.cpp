...
void save_solutions_as_json(std::string file_name,Robots &robots,double runtime,bool save_paths){
    nlohmann::json data_json;
    std::vector<std::vector<int>> path_vector(robots.size(),std::vector<int>());
    for(int i=0;i<robots.size();i++){
        for(auto &v:robots[i]->path){
            path_vector[i].push_back(v->id);
        }
    }
    int makespan,makespanLB,socLB,soc;
    evaluate_result(robots,makespan,makespanLB,soc,socLB);
    ...
    std::ofstream file_pointer(file_name);
