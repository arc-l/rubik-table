...
using EdgeProperty=boost::property< boost::edge_weight_t, int, boost::property< boost::edge_index_t, int > >;
using WeightedGraph=boost::adjacency_list<boost::vecS, boost::vecS, boost::undirectedS, boost::no_property,EdgeProperty >;
using WeightedVertex=boost::graph_traits<WeightedGraph>::vertex_descriptor;
...
double lba_sparse(std::vector<std::tuple<int,int,double>>&costEdges, std::vector<int>& assignment){
    ...
    while(hb>=lb){
        ...
        bool success=check_feasible_sparse(costEdges,assignment,m,threshold);
        if(success==true) {
            hb=thresh_id-1;
            last_good_assignment=assignment;
            infeasible=false;
...
//////////////////////////hungarian ///////////
double hungarian_s(std::vector<std::tuple<int,int,double>>&costEdges, 
    std::vector<int>& assignment){
    using costEdge=std::tuple<int,int,double>;
    std::cout<<costEdges.size()<<std::endl;
    ...
    WeightedGraph g(2*m);
    for(auto &edge:costEdges){
        add_edge(std::get<0>(edge),std::get<1>(edge)+m,EdgeProperty(-std::get<2>(edge)),g);
    }
    std::vector< WeightedVertex> mate(2*m);
    ...
    std::cout << "Matching size is " << boost::matching_size(g, &mate[0])
              << ", total weight is " << boost::matching_weight_sum(g, &mate[0])
              << std::endl;
    std::cout << std::endl;

    for (boost::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi){
        if (mate[*vi] != boost::graph_traits< WeightedGraph >::null_vertex()
            && *vi < mate[*vi])
            assignment[*vi]=mate[*vi]-m;
    }
