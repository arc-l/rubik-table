...
class Local13MP(object):
    ...
    def lbap_assign(self,agents):
        ...
        tmp_inter_goal_path=[agent.inter_goal_path for agent in agents]
        ...
        for i in row_ind:
            ...
            agents[i].inter_goal_path=tmp_inter_goal_path[col_ind[i]]
    ...
    def better_paths(self):
        for agent in self.agents:
            while len(agent.path)<agent.safe_time:
                agent.path.append(agent.path[-1])
    ...
    def reconfigure_x(self):
        ...
        for agent in self.agents:
            ...
            if xg>xs:
                ...
                for x in range(xs,xg+1):
                    ...
                    cell_id=int(x/3)
                    if cell_id not in safe_time:
                        safe_time[cell_id]=0
                    safe_time[cell_id]=max(len(agent.path), safe_time[cell_id])
        ...
        for agent in self.agents:
            xg,yg=agent.path[-1]
            cell_id=int(xg/3)
            if cell_id in safe_time:
                agent.safe_time=safe_time[cell_id]+1
            else:
                agent.safe_time=len(agent.path)
    ...
    def prepare(self,xmin,ymin,orientation='x'):
        ...
        if len(robots2)!=0:
            ...
            # start_id2=tuple([vertices.index(r.current) for r in robots2])
            start_id2=tuple([vertices.index(r.intermediate) for r in robots2])
