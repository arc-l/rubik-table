...
def save_as_json(agents,computation_time,filename,save_path=True,xmax=0,ymax=0,obstacles=[]):
    if not os.path.exists(os.path.dirname(filename)):
        try:
            os.makedirs(os.path.dirname(filename))
        except OSError as exc: # Guard against race condition
...
def save_instance_as_json(xmax,ymax,agents,filename,obstacles=[]):
    if not os.path.exists(os.path.dirname(filename)):
        try:
            os.makedirs(os.path.dirname(filename))
        except OSError as exc: # Guard against race condition
    ...
    data_dict["agents"]=[]
    for agent in agents:
        start_goal=(agent.start,agent.goal)
        data_dict["agents"].append(start_goal)
...
def save_result_as_json(agents,computation_time,filename, save_path=True,runtime=0,xmax=0,ymax=0,obstacles=[]):
    if not os.path.exists(os.path.dirname(filename)):
        try:
            os.makedirs(os.path.dirname(filename))
        except OSError as exc: # Guard against race condition
