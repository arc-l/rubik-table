...
def manhanttan(s,g):
    return abs(s[0]-s[1])+abs(g[0]-g[1])
    
def color_for_goal(g,m):
    start_color=np.array([0,0.5,0])
    start_color[0]=start_color[0]+g[0]/m
...
class Animation:
  def __init__(self, map, schedule):
    ...
    self.anim.save("demo_aorting.gif",fps=10, writer='pillow')
